...
// K_PLUGIN_FACTORY_WITH_JSON generates the required plugin metadata plumbing.
// The second argument must match the metadata.json "Id" field.
#include <KPluginFactory>
K_PLUGIN_FACTORY_WITH_JSON(RetroTermEffectFactory,
                            "metadata.json",
                            registerPlugin<RetroTermEffect>();)
...
void RetroTermEffect::loadShader()
{
    ...
    // ShaderManager::loadFragmentShader() takes a ShaderTrait bitmask describing
    ...
    m_shader = ShaderManager::instance()->loadFragmentShader(
        ShaderTrait::MapTexture, fragPath);
...
{
    if (!w) return false;
    // "*" means all windows (set by KCM when "Alle vensters" mode is selected)
